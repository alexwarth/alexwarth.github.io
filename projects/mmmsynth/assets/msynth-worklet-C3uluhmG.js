(function () {
  ...
  class Signal {
    constructor(nextSample, value = 0, noteOn, noteOff, writeSharedState) {
      this.nextSample = nextSample;
      this.value = value;
      this.noteOn = noteOn;
      this.noteOff = noteOff;
      this.writeSharedState = writeSharedState;
      this.lastFrameIdx = -1;
      Signal.newInstanceCollector?.push(this);
    }
    static {
      this.newInstanceCollector = null;
    }
    static doCollectingNewInstances(fn, collector) {
  ...
  */
  ...
  const inlinedFunctions = {
    ...
    norm: (value, min = "-1", max = "1") => `((value, min, max) => max <= min ? 0 : (value - min) / (max - min))(${value}, ${min}, ${max})`,
    /** [0, 1] -> [min, max], linearly */
    lscale: (value, min, max) => `((value, min, max) => value * (max - min) + min)(${value}, ${min}, ${max})`,
    /** [-1, 1] -> [min, max], linearly */
    declip: (value, min, max) => `((value, min, max) => value * (max - min) + min)((${value} - 1) / 2, ${min}, ${max})`,
    /** [0, 1] -> [min, max], exponentially */
    escale: (s, min, max) => `((value, min, max) => {
      if (min === 0) {
        min = 0.001;
      }
    ...
    // ----- frequency helpers -----
    semitones: (n) => `Math.pow(2, ${n} / 12)`,
    quantizeFreq: (f, b = "440") => `((f, b) => (b * Math.pow(2, Math.round(12 * Math.log2(f / b)) / 12)))(${f}, ${b})`,
    // ----- conditionals -----
    ifPos: (s, t, f) => `(${s} >= 0 ? ${t} : ${f})`,
    switch: (s, neg, zero, pos) => `((s, neg, zero, pos) => s === 0 ? zero : s < 0 ? neg : pos)(${s}, ${neg}, ${zero}, ${pos})`
  };
  ...
  const SAMPLE_RATE_RECIP = 1 / SAMPLE_RATE;
  const TAU = 2 * Math.PI;
  ...
  function exponential(input, t, legato) {
    let lastValue = input.value;
    let jumping = false;
    return Signal.new({
      nextSample() {
  ...
  function toFrequency(midiNote) {
    return 440 * Math.pow(2, (midiNote - 69) / 12);
  }
  function interpolatedRead(xs, idx) {
  ...
  const INTERPOLATION_RECIP = 1 / INTERPOLATION;
  const SAMPLE_RATE_INTERPOLATION_RECIP = 1 / (SAMPLE_RATE * INTERPOLATION);
  const RESONANCE_MULTIPLIER = 1.8;
  const MIN_CUTOFF_FREQ = 5;
  const MAX_CUTOFF_FREQ = SAMPLE_RATE * 0.425;
  const INPUT_THRESHOLD = 1e-5;
  const STATE_DECAY = 0.95;
  function ladder(mode, _input, _cutoffFreq, _q) {
    let z0 = [0, 0, 0, 0];
    let z1 = [0, 0, 0, 0];
    let alpha = 1;
    let k = 1;
    ...
    function setPassbandGain(value) {
    ...
    function weightedSumForCurrentMode(stageOuts0, stageOuts1, stageOuts2, stageOuts3, stageOuts4) {
      switch (mode) {
        case "lp24":
          return stageOuts4;
        case "lp12":
  ...
  const msynthLib = {
    ...
    pwm: (f = scalar(440), m = scalar(0.5), sync = null) => {
      let phase = 0;
      let prevSyncValue = 0;
      return Signal.new(() => {
        const currSyncValue = sync?.value ?? 0;
        if (prevSyncValue < 0 && currSyncValue >= 0) {
          const currSyncValue2 = sync?.value ?? 0;
          if (prevSyncValue < 0 && currSyncValue2 >= 0) {
            const r = -prevSyncValue / (currSyncValue2 - prevSyncValue);
    ...
    lglide: (s, t = scalar(0.1)) => linear(s, t),
    /** glide exponentially (t is time for the signal to move 1 octave, in seconds) */
    eglide: (s, t = scalar(1)) => exponential(s, t, false),
    legato: (s, t = scalar(1)) => exponential(s, t, true),
    // ----- envelopes -----
    ad: ad,
    adsr: adsr,
    // ----- filters -----
    lpf: (s, cf, q = scalar(0.2)) => ladder("lp24", s, cf, q),
    lpf12: (s, cf, q = scalar(0.2)) => ladder("lp12", s, cf, q),
    lpf24: (s, cf, q = scalar(0.2)) => ladder("lp24", s, cf, q),
    hpf: (s, cf, q = scalar(0.2)) => ladder("hp24", s, cf, q),
    hpf12: (s, cf, q = scalar(0.2)) => ladder("hp12", s, cf, q),
    hpf24: (s, cf, q = scalar(0.2)) => ladder("hp24", s, cf, q),
    bpf: (s, cf, q = scalar(0.2)) => ladder("bp24", s, cf, q),
    bpf12: (s, cf, q = scalar(0.2)) => ladder("bp12", s, cf, q),
    bpf24: (s, cf, q = scalar(0.2)) => ladder("bp24", s, cf, q),
    // ----- effects -----
    delay: delay,
    // ----- other helpers -----
    latch: (s, useLastValue) => {
      let lastValue = 0;
      return Signal.new(() => {
...
`);
  ...
  class Prog extends ANode {
    ...
    checkCalls(inlinedFunctions2, lib2) {
      for (const decl of this.decls) {
        decl.checkCalls(this.mdecls, inlinedFunctions2, lib2);
      }
    }
    checkRefs(builtInSignals2, paramNames) {
      ...
      for (const name of refs) {
        ...
        } else if (builtInSignals2.hasOwnProperty(name)) {
          builtins.add(name);
    ...
    trans(refs, inlinedFunctions2, paramNames) {
      const haveSignal = (name) => this.sdecls.some((d) => d.name === name);
      let outputs;
      ...
      ${[...paramNames.entries()].filter(([name, n]) => refs.params.includes(n)).map(([name, n]) => `const __${name} = __param${n};`).join("\n")}
      ${refs.builtins.map((name) => `const __${name} = __.${name}();`).join("\n")}
      ${this.sdecls.map((d) => `let __${d.name};`).join("\n")}
      ${this.sdecls.map((d) => d.trans(this.mdecls, inlinedFunctions2)).join("\n")}
      ${this.sdecls.map((d) => `__${d.name} = __${d.name}.force();`).join("\n")}
      return [${outputs.join(", ")}];
    })()`;
    }
  }
  class ADecl extends ANode {
    constructor(name, rhs, interval) {
      super(interval);
      this.name = name;
      this.rhs = rhs;
    }
    checkCalls(mdecls, inlinedFunctions2, lib2) {
      this.rhs.checkCalls(mdecls, inlinedFunctions2, lib2);
    }
  }
  class SDecl extends ADecl {
    ...
    trans(mdecls, inlinedFunctions2) {
      const signals = [];
      const expr = this.rhs.trans("signal", signals, mdecls, inlinedFunctions2);
      const signal = signals.length === 0 ? expr : `{
      ${signals.map((code2, idx) => `const tmp${idx} = ${code2};`).join("\n")}
      return ${expr};
    }`;
  ...
  class MDecl extends ADecl {
    constructor(name, argNames, rhs, interval) {
      super(name, rhs, interval);
  ...
  class AExpr extends ANode {
    constructor(interval, children = []) {
      super(interval);
      this.children = children;
    }
    checkCalls(mdecls, inlinedFunctions2, lib2) {
      for (const child of this.children) {
        child.checkCalls(mdecls, inlinedFunctions2, lib2);
      }
    }
    addRefs(refs) {
      for (const child of this.children) {
  ...
  class Call extends AExpr {
    constructor(name, args, interval) {
      super(interval, args);
      this.name = name;
      this.args = args;
    }
    checkCalls(mdecls, inlinedFunctions2, lib2) {
      super.checkCalls(mdecls, inlinedFunctions2, lib2);
      const isDeclared = mdecls.some((d) => d.name === this.name) || inlinedFunctions2.hasOwnProperty(this.name) || lib2.hasOwnProperty(this.name);
      if (!isDeclared) {
    ...
    trans(want, signals, mdecls, inlinedFunctions2) {
      const macro = mdecls.find((d) => d.name === this.name);
      if (macro) {
        return macro.expand(this).trans(want, signals, mdecls, inlinedFunctions2);
      }
      let call;
      if (inlinedFunctions2.hasOwnProperty(this.name)) {
        const args = this.args.map((arg) => arg.trans("number", signals, mdecls, inlinedFunctions2));
        call = inlinedFunctions2[this.name](...args);
        if (want === "signal") {
        ...
        const args = this.args.map((arg) => arg.trans("signal", signals, mdecls, inlinedFunctions2));
  ...
  const s = grammar.createSemantics().addOperation("toAst", {
    Prog(decls) {
  ...
  function compile(input, paramNames) {
    const mr = grammar.match(input);
    if (mr.failed()) {
      const expected = mr.getRightmostFailures().map((f) => f.type === "string" ? `"${f.text}"` : f.text);
    ...
    const prog = s(mr).toAst();
    prog.checkDuplicateDeclarations();
    prog.checkCalls(inlinedFunctions, msynthLib);
    const refs = prog.checkRefs(builtInSignals, paramNames);
    const code2 = prog.trans(refs, inlinedFunctions, paramNames);
    console.log("compiled code:\n", code2);
    return () => evalCompiledCode(code2);
  }
  function evalCompiledCode(code) {
    return eval(code);
  }

  class Synth {
    constructor(outs, allSignals, params) {
      this.outs = outs;
  ...
  class Msynth extends AudioWorkletProcessor {
    constructor() {
      super();
      this.synth = null;
      this.stopped = false;
      this.port.onmessage = (msg) => this.onMessage(msg.data);
    }
    ...
    sendMessage(msg, transferObjects = []) {
      ...
      switch (msg.command) {
        case "load patch": {
          try {
            ...
            this.synth = new Synth(outs, allSignals, new Float32Array(msg.params));
          } catch (e) {
